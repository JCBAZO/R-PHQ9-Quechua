#### PHQ-9 Quechua: CFA, CFA Multigroups & MIMIC
...
#Model across Quechua Variants
  measurementInvariance(model=PHQ9_R.model, data=work_data, group = "VAR", strict = "TRUE", parameterization = "theta",
                      ordered=c("P1", "P2", "P3", "P4", "P5", "P6", "P7", "P8", "P9"))
...
###MIMIC MODELS EDAD (MEASUREMENT MODEL (MM), AGE->DIMENSIONS (AD, intercept model), AGE->INDICATORS (AI, saturated model))
#### PHQ-9
...
###MIMIC MODELS Estado Civil (MEASUREMENT MODEL (MM), EstCiv->DIMENSIONS (ED, intercept model), EstCiv->INDICATORS (EI, saturated model))
#### PHQ-9
...
###MIMIC MODELS Grado Instruccion (MEASUREMENT MODEL (MM), GradInst->DIMENSIONS (GD, intercept model), GradInst->INDICATORS (GI, saturated model))
#### PHQ-9
